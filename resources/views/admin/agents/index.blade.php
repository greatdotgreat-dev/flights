                    ...
                    @foreach($agents as $agent)
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                <form action="{{ route('admin.agents.toggleStatus', $agent->id) }}" method="POST" class="d-inline">
    @csrf
    <button type="submit" 
        ...
        {{ $agent->is_active ? 'Deactivate' : 'Activate' }}
                        ...
                        <!-- Modal -->
                        <div class="modal fade" id="toggleStatusModal{{ $agent->id }}" tabindex="-1" aria-labelledby="toggleStatusModalLabel{{ $agent->id }}" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="toggleStatusModalLabel{{ $agent->id }}">{{ $agent->is_active ? 'Deactivate' : 'Activate' }} Agent</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        Are you sure you want to {{ $agent->is_active ? 'deactivate' : 'activate' }} this agent?    
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                                        <form action="{{ route('admin.agents.toggleStatus', $agent->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            <button type="submit" class="btn btn-{{ $agent->is_active ? 'danger' : 'success' }}">
                                                {{ $agent->is_active ? 'Deactivate' : 'Activate' }}
                                            </button>
                                        </form>
                                    </div>
